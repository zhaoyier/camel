...
func (sc *ServerConn) Write(message ZMessage) error {
...
// Write writes a message to the client.
func (cc *ClientConn) Write(message ZMessage) error {
...
// Write writes a message to the client.
func (sc *GatewayConn) Write(message ZMessage) error {
	return asyncWrite(sc, message)
}

// Close gracefully closes the server connection. It blocked until all sub
// go-routines are completed and returned.
func (sc *GatewayConn) Close() {
	sc.once.Do(func() {
		holmes.Infof("conn close gracefully, <%v -> %v>\n", sc.rawConn.LocalAddr(), sc.rawConn.RemoteAddr())
		...
		sc.mu.Lock()
		sc.cancel()
		//TODO pending := sc.pending
		sc.pending = nil
		sc.mu.Unlock()

		// clean up pending timers
		//TODO for _, id := range pending {
		// 	//TODO sc.CancelTimer(id)
		// }
...
// SetName sets name of server connection.
func (sc *GatewayConn) SetName(name string) {
	sc.mu.Lock()
	defer sc.mu.Unlock()
	sc.name = name
}
...
// RunEvery runs a callback on every interval time.
func (sc *GatewayConn) RunEvery(interval time.Duration, callback func(time.Time, WriteCloser)) int64 {
	id := runEvery(sc.ctx, sc.netid, sc.belong.timing, interval, callback)
	if id >= 0 {
		//TODO sc.AddPendingTimer(id)
	}
	return id
}

// SetHeartBeat sets the heart beats of server connection.
func (sc *GatewayConn) SetHeartBeat(heart int64) {
	sc.mu.Lock()
	defer sc.mu.Unlock()
	sc.heart = heart
}
...
func asyncWrite(c interface{}, m ZMessage) (err error) {
	defer func() {
		if p := recover(); p != nil {
			err = ErrServerClosed
		}
	...
	select {
	//case sendCh <- pkt:	//TODO 封装消息,
	case sendCh <- Encode(m):
		err = nil
	default:
		err = ErrWouldBlock
	}
...
func readLoop(c WriteCloser, wg *sync.WaitGroup) {
	...
	var (
		...
		cDone <-chan struct{}
		sDone <-chan struct{}
		//TODO setHeartBeatFunc func(int64)
	...
	case *ClientConn:
		...
		cDone = c.ctx.Done()
		sDone = nil
		//TODO setHeartBeatFunc = c.SetHeartBeat
	...
	for {
		...
		case <-sDone: // server closed
			...
			return
		default:
			//TODO 判断消息来源User/Server
			scanner := Scanner(rawConn)
			for scanner.Scan() {
				msg := new(ZMessage)
				msg.Unpack(bytes.NewReader(scanner.Bytes()))
				fmt.Printf("===>>read messge: %+v\n", msg)
			...
			// 	return
			// }
			//TODO setHeartBeatFunc(time.Now().UnixNano())
...
func (sc *GatewayConn) transmit2Server(netid int64, msg *ZMessage) {
	...
	id := sc.netid % int64(len(manager.agents))
	msg.Source = int16(Source_Gateway)
	msg.ProxyID = sc.netid
	buf := new(bytes.Buffer)
	msg.Pack(buf)
	agent := manager.agents[id]
	agent.conn.Write(buf.Bytes())

	// 封装消息
}

func Dispatch(addr net.Addr, agents []*agent) *agent {
	if len(agents) <= 0 {
		return nil
	}
	v := int(crc32.ChecksumIEEE([]byte(addr.String())))
	if v < 0 {
		v = -v
	}
	remainder := v % len(agents)
	return agents[remainder]
}

func (sc *GatewayConn) server2Register(msg *ZMessage) {
	switch msg.ReqID {
	case 1: //ping
		sc.rawConn.Write([]byte("pong"))
	case 2: //注册服务
		fmt.Println("====>>>register server")
		req := &RegisterServerReq{}
		if err := proto.Unmarshal(msg.Data, req); err == nil {
			fmt.Printf("proto unmarshal msg:%+v\n", req)
			manager, ok := sc.belong.proxy[req.GetModulo()]
			if !ok {
				manager.agents = make([]*agent, 0, 2)
				sc.belong.proxy = make(map[int32]agentManager)
			}
			manager.agents = append(manager.agents, &agent{
				address: sc.rawConn.RemoteAddr().String(),
				conn:    sc.rawConn,
			})
			sc.belong.proxy[req.GetModulo()] = manager
			fmt.Printf("proxy server len: %d|%d\n", len(sc.belong.proxy), len(sc.belong.proxy[req.Modulo].agents))
		} else {
			fmt.Printf("proto unmarshal error:%s\n", err.Error())
		}
	}
}
